...
function ProgressBar(fmt, options) {
  ...
  this.chars = {
    ...
    cursor     : options.cursor || options.complete || '=',
...
ProgressBar.prototype.render = function (tokens) {
  ...
  cursor = (completeLength === 0 ||  this.curr >= this.total) ? '' : this.chars.cursor;
  complete = Array(completeLength + 1 - cursor.length).join(this.chars.complete);
  incomplete = Array(width - completeLength + 1).join(this.chars.incomplete);

  /* fill in the actual progress bar */
  str = str.replace(':bar', complete + cursor + incomplete);
